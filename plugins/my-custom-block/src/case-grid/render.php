...
$is_carousel    = !empty($attributes['isCarousel']);
...
if ($is_carousel) :
	// Carousel mode: fetch ALL matching cases up front. Pagination happens
	// entirely client-side via the Interactivity API, so paging never makes
	// another request or hits the database again.
	$args = array(
	...
	$visible_posts = array_slice($all_posts, 0, max(1, $posts_per_page));
	...
	if ($total_posts > 0) : ?>
		<div
			<?php echo get_block_wrapper_attributes(['class' => 'case-grid flex flex-col items-center w-full']); ?>
			<?php echo $style; ?>
			data-wp-interactive="case-grid"
			data-wp-context='<?php echo wp_json_encode($context); ?>'
		>
			<div class="grid grid-cols-1 md:grid-cols-3 gap-8 w-full mb-12 relative">
				<!-- Loading Spinner -->
				<div
				...
				<div
					class="grid grid-cols-1 md:grid-cols-3 gap-8 w-full col-span-3 overflow-hidden"
					data-case-grid-grid
				>
					<!-- This template is expanded on the server AND client -->
					<template
						data-wp-each--case="state.posts"
						data-wp-key="context.case.id"
					>
						<a data-wp-bind--href="context.case.permalink" class="group hover:no-underline no-underline!">
							<article class="flex flex-col h-full">
								<div
									class="mb-6 aspect-square w-full overflow-hidden"
									data-wp-bind--hidden="!context.case.featuredImage"
									hidden
								>
									<img
										data-wp-bind--src="context.case.featuredImage"
										alt=""
			...
			<!-- Navigation Controls (same markup/behavior as Team Gallery) -->
			<?php if ($max_pages > 1) : ?>
				<div class="flex items-center gap-12 mt-4 mb-12 relative z-10">
					<button
					...
					>
						<svg width="60" height="52" viewBox="0 0 80 70" fill="none" xmlns="http://www.w3.org/2000/svg">
							<g clip-path="url(#cg_clip0_right)">
	...
	<?php else : ?>
		<p>Inga kundcase hittades.</p>
	<?php endif;

else :
	// Static grid mode (unchanged existing behavior).
	$args = array(
		'post_type'      => 'customer_case',
