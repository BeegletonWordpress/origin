...
export const SmallRingShapeSVG = ({
	scaleX = 100,
	scaleY = 120,
	offsetX = -4,
	offsetY = -0.5,
	...
	<svg
		...
		fill="none"
		preserveAspectRatio="none"
		className="absolute inset-0 w-full h-full z-0 translate-x-7 translate-y-4 scale-x-110"
		style={{
			transform: `scale(${scaleX / 100}, ${
				scaleY / 100
			}) translate(${offsetX}rem, ${offsetY}rem)`,
			transformOrigin: "center",
			color: color || "inherit",
		}}
	>
		<path
			className="handdrawn-ring-path"
...
registerBlockType(metadata.name, {
	...
	save: function Save({ attributes }) {
		...
		const blockProps = useBlockProps.save({
			className: "handdrawn-header w-fit pb-0!",
			style: {
				marginBottom: `${underlineSpacing}rem`,
			},
		});
